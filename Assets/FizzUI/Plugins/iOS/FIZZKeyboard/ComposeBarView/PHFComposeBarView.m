...
{
    return textView.text.length + (text.length - range.length) <= _maxCharCount;
}
...
- (void)setUtilityButtonImage:(UIImage *)image {
    [[self utilityButton] setImage:[UIImage imageNamed:@"FizzResourceBundle.bundle/fizz_smiley.png"] forState:UIControlStateNormal];
...
//        [_button setTitleEdgeInsets:UIEdgeInsetsMake(0.5f, 0, 0, 0)];
        [_button setAutoresizingMask:UIViewAutoresizingFlexibleTopMargin];
//        [_button setTitle:[self buttonTitle] forState:UIControlStateNormal];
        [_button setImage:[UIImage imageNamed:@"FizzResourceBundle.bundle/sendButton.png"] forState:UIControlStateNormal];
...
- (UIView *)textContainer {
    if (!_textContainer) {
        CGRect textContainerFrame = CGRectMake(kHorizontalSpacing,
                                               ...
                                               [self bounds].size.width - kUtilityButtonWidth - (kButtonHeight * 1.363) - (kHorizontalSpacing * 3),
        ...
        [_textContainer setBackgroundColor:[UIColor whiteColor]];
...
- (UIButton *)utilityButton {
    if (!_utilityButton) {
        ...
        [_utilityButton setFrame:CGRectMake([self bounds].size.width - kUtilityButtonWidth - (kButtonHeight * 1.363) - (kHorizontalSpacing * 2),
...
//    [[self button] sizeToFit];
    CGFloat widthDelta = (previousButtonFrame.size.height * 1.363) - previousButtonFrame.size.width;

    newButtonFrame.size.width += widthDelta;
    newButtonFrame.origin.x -= widthDelta + kButtonTouchableOverlap;
//    [[self button] setFrame:newButtonFrame];

    textContainerFrame.size.width -= widthDelta + 2 * kButtonTouchableOverlap;
    [[self textContainer] setFrame:textContainerFrame];

    charCountLabelFrame.origin.x = textContainerFrame.origin.x + textContainerFrame.size.width - 2 * kButtonTouchableOverlap;
...
- (void)setup {
    ...
    UnitySetViewTouchProcessing(_topLineView, touchesIgnored);
    UnitySetViewTouchProcessing(_backgroundView, touchesIgnored);
    UnitySetViewTouchProcessing(_textContainer, touchesIgnored);
    UnitySetViewTouchProcessing(_button, touchesIgnored);
